...
func (d *CacheStore) Read(id interface{}) (interface{}, error) {
	...
	logger.Infof("Read name=%v id=%v value:%v err:%v", d.options.uniqName, id, rst, err)
	if err != nil {
...
func (d *CacheStore) Save(id, value interface{}) error {
	...
	if err != nil {
		...
		//logger.Infof("Save insert name=%v id=%v value:%v err:%v", d.options.uniqName, id, value, err)
		if err != nil {
		...
		//logger.Infof("Save update name=%v id=%v value:%v err:%v", d.options.uniqName, id, value, err)
	}
	return nil
}
...
func (d *CacheStore) syncHash() error {
	...
	for key, version := range ssm {
		...
		allCount++
		newCount += insert
		logger.Infof("SyncHash succ key:%v version:%v", key, version)
	}
	logger.Errorf("SyncHash succ name:%v total:%v need:%v all:%v new:%v", d.options.uniqName, total, need, allCount, newCount)
	return nil
}
	...
	if err == redis.Nil {
		...
		if d.options.mongo == nil {
			...
			if rst == nil {
				...
				logger.Infof("getEx DefaultData name=%v key=%v id=%v", d.options.uniqName, key, id)
				//默认数据不落地
		...
		logger.Errorf("getEx err name=%v key=%v value=%v err=[%v]", d.options.uniqName, key, val, err)
		return nil, err
	} else {
...
func (d *CacheStore) setEx(id, value interface{}, noExpired bool, noSync bool) error {
	...
	if noExpired {
		...
		if err != nil {
			logger.Errorf("setEx err keepttl name=%v key=%v valule=%v err:%v", d.options.uniqName, key, saveData, err)
		}
		if noSync {
			return err
		}
...
func (d *CacheStore) syncOneHash(key string, version string) (int, error) {
	...
	logger.Infof("syncOne name=%v key=%v id=%v value:%v err:%v", d.options.uniqName, key, val.Interface(), rst, err)
	syncKey := d.getSyncKeyHash()
